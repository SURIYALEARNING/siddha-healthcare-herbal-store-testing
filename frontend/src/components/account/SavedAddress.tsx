...
export default function SavedAddress({ user }: SavedAddressProps) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-base font-bold text-gray-850 uppercase tracking-wider border-b border-gray-50 pb-2.5">{t("user.savedAddresses")}</h3>
      ...
      {user.address ? (
        <div className="border border-gray-150 rounded-2xl p-5 space-y-3 bg-slate-50 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start z-10 relative">
            <div>
              <span className="text-[10px] bg-siddha-light text-siddha-dark font-bold px-2 py-0.5 rounded uppercase tracking-wider">{t("checkout.shippingAddress")}</span>
              ...
              <p className="text-xs font-medium text-gray-400 font-mono mt-2 flex items-center">
                ...
                {t("auth.phone")}: {user.mobileNumber}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center py-12 border border-dashed border-gray-250 rounded-2xl p-6">
          ...
          <p className="text-xs text-gray-400 mt-2">{t("user.savedAddresses")}</p>
        </div>
      )}
    </div>
  );
