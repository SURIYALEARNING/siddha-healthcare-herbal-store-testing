...
interface ShippingTableProps {
  ...
  onCreateShiprocketOrder: (orderId: string) => Promise<string | null>;
...
export default function ShippingTable({
  ...
  const handleConfirmOrder = async (orderId: string) => {
    await doAction(`confirm-${orderId}`, () => onConfirmOrder(orderId));
    const shipmentId = await onCreateShiprocketOrder(orderId);
    if (shipmentId) {
      await doAction(`awb-${orderId}`, () => onGenerateAWB(orderId, shipmentId));
    }
  };
                  ...
                  <div className="flex items-center justify-end gap-1 flex-wrap">
                    {sw <= statusWeight("PAID") && (
                      <button
                        ...
                        disabled={isDoing("confirm")}
                        className="px-3 py-1.5 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white rounded-lg text-[10px] font-bold cursor-pointer transition-colors flex items-center gap-1"
                      >
                        {isDoing("confirm") ? "..." : <><CheckCircle className="w-3 h-3" /> Confirm</>}
                      </button>
                    )}
